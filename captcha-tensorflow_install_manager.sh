...
#6.编译.../captcha-tensorflow/docker/Dockerfile.captcha-tensorflow.../captcha-tensorflow/docker/Dockerfile.captcha-tensorflow
#7.启动镜像，并加载volume
#
#   ../captcha-tensorflow_workspace 目录结构
#	|--data	#模型
#	|--captcha-tensorflow #代码
#	|--img	#检测数据
#
# 注意：
...
IMAGENAME=captcha-tensorflow_dockerimage
...
do
	...
	echo "For Example:/home/$USER/DataCenterProject/captcha-tensorflow_workspace "
...
echo "############################################################################"
echo "##									##"
echo "##	检测是否有captcha-tensorflow_DockerImage:py27镜像	##"
echo "##	编译.../captcha-tensorflow/docker/Dockerfile.captcha-tensorflow		##"
echo "##	或.../captcha-tensorflow/docker/Dockerfile.captcha-tensorflow		##"
echo "##									##"
echo "############################################################################"
...
then
	...
	then
		#sudo docker build -t $IMAGENAME:$IMAGETAGCPU -f ${VOLUMEPATH}/Dockerfile.captcha-tensorflow ${VOLUMEPATH}
		sudo docker build -t $IMAGENAME:$IMAGETAGCPU -f ${VOLUMEPATH}/docker/cpu/Dockerfile.captcha-tensorflow.cpu ${VOLUMEPATH}/docker/cpu
	else 
...
then
	...
	then
		...
		sudo docker build -t $IMAGENAME:$IMAGETAGGPU -f ${VOLUMEPATH}/docker/gpu/Dockerfile.captcha-tensorflow.gpu ${VOLUMEPATH}/docker/gpu
	else 
...
if [ "$CHIPMODE"x = "gpu"x ];
then
	sudo docker run --runtime=nvidia -it --volume ${VOLUMEPATH}:/workspace -p $HOSTPORT_FLASK:$OUTPORT_FLASK $IMAGENAME:$IMAGETAGGPU
	#sudo nvidia-docker run -it --volume ${VOLUMEPATH}:/workspace -p $HOSTPORT:$PORT $IMAGENAME:$IMAGETAGGPU
	sudo docker run --runtime=nvidia -it --volume ${VOLUMEPATH}:/workspace -p $HOSTPORT_FLASK:$OUTPORT_FLASK $IMAGENAME:$IMAGETAGGPU /bin/bash
